...
class SocialWithLoginController extends Controller
{
    ...
    public function callbackFromGoogle()
    {
        try {
            ...
            $users = User::where('email', $user->getEmail())->where('status', '1')->first();
            if (empty($users->email) && empty($users->id)) {
                Session::flash('error', 'Not exist account!');
                return redirect('/');
            } else {
    ...
    public function linkedinCallback()
    {
        try {
            ...
            if (empty($linkedinUser->email) && empty($linkedinUser->id)){
                Session::flash('error', 'Not exist account!');
                return redirect('/');
            }else{
